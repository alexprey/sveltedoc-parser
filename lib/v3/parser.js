...
class Parser extends EventEmitter {
    ...
    emitEventItem(event, parseContext) {
        const item = Object.assign({}, utils.getComment(event.node, { defaultVisibility: 'public' }), {
            name: event.name
        });
    ...
    emitImportedComponentItem(component, parseContext) {
        const item = Object.assign({}, utils.getComment(component.node, { defaultVisibility: 'private' }), {
            name: component.name,
    ...
    emitGlobalComment(comment) {
        if (utils.isTopLevelComment(comment)) {
            if (this.features.includes('description')) this.emit('description', comment.description);
            if (this.features.includes('keywords')) this.emit('keywords', comment.keywords);
        }
    }
    ...
    parseBodyRecursively(rootNode, parseContext, level) {
        ...
        nodes.forEach((node, index) => {
            if (index === 0) {
                const firstComment = utils.getComment(node, { useTrailing: false, useFirst: true });
    ...
    parseTemplate() {
        ...
        const parser = new HtmlParser({
            ...
            onopentag: (tagName, attrs) => {
                const isTopLevelElement = !['style', 'script'].includes(tagName) && parser._stack.length === 1;

                if (isTopLevelElement && lastComment) {
