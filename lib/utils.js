...
const getComment = (property, {
    ...
    if (property) {
        if (useLeading && property.leadingComments && property.leadingComments.length > 0) {
            const leadingCommentIndex = useFirst ? 0 : property.leadingComments.length - 1;

            lastComment = property.leadingComments[leadingCommentIndex].value;
        }

        if (useTrailing && property.trailingComments && property.trailingComments.length > 0) {
            const trailingCommentIndex = useFirst ? 0 : property.trailingComments.length - 1;

            lastComment = property.trailingComments[trailingCommentIndex].value;
        }
    }
