...
class Parser extends EventEmitter {
    ...
    parseBodyRecursively(rootNode, parseContext, level) {
        ...
        nodes.forEach(node => {
            if (index === 0) {
                const topLevelComment = utils.getComment(node);
                if (topLevelComment.keywords.some((keyword) => keyword.name === 'module')) {
                  this.emit('description', topLevelComment.description);
                  this.emit('keywords', topLevelComment.keywords);
                }
              }
